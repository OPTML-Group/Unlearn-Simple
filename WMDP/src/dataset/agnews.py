...
class AGNews(BaseDataset):
    def __init__(self, dataset_name, with_retain=False, if_llama=False, subset=None):
        ...
        self.subset = subset
        self.instruction = "Analyze the sentiment of the input, and respond only positive or negative"
    ...
    def get_dataset(self):
        if self.subset == "retain":
            ...
            train_dataset = load_dataset(
                "ag_news", cache_dir="./.cache", split="train"
            )
        
        test_dataset = load_dataset(
            "ag_news", cache_dir="./.cache", split="test"
        )
    ...
    def __preprocess__(self, tokenizer):
        ...
        def preprocess(examples):
            ...
            # print(examples)
            for question, label in zip(examples["text"], examples["label"]):
                if label == 0: 
                    answer = "World"
                elif label == 1:
                    answer = "Sports"
