from data_module import TextForgetDatasetQA, TextForgetDatasetDPOQA, TextForgetDatasetKTOQA, TextDatasetQA
...
def main(cfg):
    ...
    results = trainer.get_all_loss(torch_format_dataset)
    import json

    # os.makedirs(os.path.dirname(cfg.save_dir), exist_ok=True)
    # print(f"Saving to: {cfg.save_dir}")
    # with open(str(cfg.save_dir), 'w') as f:
    #     json.dump(results, f, indent=4)

    # save_dir = os.path.dirname(cfg.save_dir)
    # os.makedirs(save_dir, exist_ok=True)

    with open('origin_retain.json', 'w') as f:
        json.dump(results, f, indent=4)
