from data_module import TextForgetDatasetQA, TextForgetDatasetDPOQA, TextForgetDatasetKTOQA, WMDPDataset, UnlearnDataset, unlearncollector
# StackUnlearnDataset, StackDataCollator
from dataloader import CustomTrainerForgetting, custom_data_collator_forget_wmdp
import torch
...
def main(cfg):
    ...
    dataset = WMDPDataset(subset="forget")
    dataset = dataset.build_dataset(tokenizer)
    forget_dataset = dataset["train"]

    dataset = WMDPDataset(subset="retain")
    dataset = dataset.build_dataset(tokenizer)
    retain_dataset = dataset["train"]

    # torch_format_dataset = StackUnlearnDataset(
    #     remain=retain_dataset, forget=forget_dataset, main_key="forget"
    # )
    torch_format_dataset = UnlearnDataset(
    ...
    trainer = CustomTrainerForgetting(
        ...
        args=training_args,
        # data_collator=StackDataCollator(),
        # data_collator=unlearncollector,
        data_collator=custom_data_collator_forget_wmdp,
